...
public class Ball implements Comparable<Ball>{
	...
	public Ball(float x, float y, float radius, float mass, int ballCount)
	{
	...
	public boolean urn() {
		return position.getX() > 185 && position.getX() < 395 && position.getY() < 400 && position.getY() > 160;
	}
	...
	public void draw(Graphics2D g2)
	{
		...
		if (ballNumber >= 10) {
			g2.drawString(ballNumber + "", position.getX() - getRadius()/2, position.getY() + getRadius()/5);
		} else {
			g2.drawString(ballNumber + "", position.getX() - getRadius()/5, position.getY() + getRadius()/5);
		}
	}
